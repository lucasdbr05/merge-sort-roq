...
Definition sorted_pair_lst (p: list nat * list nat) := Sorted le (fst p) /\ Sorted le (snd p).
...
Theorem sorted_implies_head_is_smallest : forall l, Sorted le l -> first_is_smallest l.
Proof.
  intros l H.
  induction H as [| x l Hsorted IH Hhd].
  - unfold first_is_smallest. trivial.
  - unfold first_is_smallest.
    intros y Hin.
    destruct l as [| z l'].
    + simpl in Hin. destruct Hin.
    + simpl in Hin. destruct Hin as [Hyz | Hintl].
      * subst y. inversion Hhd; subst. assumption.
      * inversion Hhd; subst.
        unfold first_is_smallest in IH.
        specialize (IH y Hintl).
        transitivity z; assumption.
Qed.

Theorem tail_of_ord_l_is_sorted : forall (h1 : nat) (l1 : list nat), Sorted le (h1 :: l1) -> Sorted le l1.
Proof.
  intros h1 l1 H.
...
Lemma x_leq_all_in_l_implies_x_concat_l_is_sorted : forall x l, (forall y, In y l -> x <= y) -> Sorted le l -> Sorted le (x :: l).
Proof.
  intros x l H_le H_sorted.
  constructor.
  - assumption.
  - destruct l as [| hd tl].
    + constructor.
    + constructor. apply H_le. left. reflexivity.
Qed.

Theorem merge_ordena: forall p, sorted_pair_lst p -> Sorted le (merge p).
Proof.
  intros p [H1 H2].
  functional induction (merge p).
  - simpl. assumption.
...
Lemma mergesort_ordena: forall l, Sorted le (mergesort l).
Proof.
  intros l.
  functional induction (mergesort l).
  - apply Sorted_nil.
...
Theorem mergesort_correto: forall l, Sorted le (mergesort l) /\ Permutation (mergesort l) l.
Proof.
  intros l.
  split.
